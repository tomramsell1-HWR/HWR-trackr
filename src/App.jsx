  ...
  return (
    <div style={{ marginBottom: 14, borderRadius: 18, overflow: "hidden", boxShadow: "0 2px 16px #00000009" }}>
      <div onClick={() => setOpen((o) => !o)} style={{
        ...
        <span style={{ color: color.base, fontSize: 22, fontWeight: 700, lineHeight: 1 }}>{open ? "−" : "+"}</span>
      </div>
...
function FeedSection({ entries, onAdd, onDelete }) {
  ...
  return (
    <Section title="Feeds" emoji="🍼" color={T.feed} count={entries.length}>
      ...
      <Field label="Notes">
        <Txt placeholder="Any observations about how she fed…" value={notes} onChange={(e) => setNotes(e.target.value)} />
      </Field>
        ...
        <div style={{ marginTop: 16 }}>
          {entries.slice(0, 10).map((e) => (
            <EntryCard key={e.id} entry={e} color={T.feed} onDelete={onDelete}
              summary={[e.what, e.size && `(${e.size})`, e.duration_mins && `${e.duration_mins} min`].filter(Boolean).join(" · ")} />
          ))}
        </div>
      )}
    </Section>
...
function NappySection({ entries, onAdd, onDelete }) {
  ...
  return (
    <Section title="Nappies" emoji="💩" color={T.poo} count={entries.length}>
      <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
        {tabBtn("poo", `💩 Poos (${poos.length})`)}
        {tabBtn("wee", `💧 Wees (${wees.length})`)}
      </div>
      ...
      {tab === "poo" && <>
        ...
        <Field label="Notes">
          <Txt placeholder="Any concerns to note…" value={pooNotes} onChange={(e) => setPooNotes(e.target.value)} />
        </Field>
          ...
          <div style={{ marginTop: 16 }}>
            ...
            {poos.slice(0, 8).map((e) => (
              <EntryCard key={e.id} entry={e} color={T.poo} onDelete={onDelete}
                summary={[e.colour, e.consistency, e.size && `(${e.size})`].filter(Boolean).join(" · ") || "Poo logged"} />
            ))}
          </div>
        )}
      </>}
      ...
      {tab === "wee" && <>
        {weesToday > 0 && (
          <div style={{ background: T.poo.light, borderRadius: 10, padding: "8px 14px", marginBottom: 14, fontSize: 13, fontWeight: 700, color: T.poo.dark }}>
            💧 {weesToday} wee{weesToday !== 1 ? "s" : ""} logged today
          </div>
        )}
        ...
        <Field label="Notes (optional)">
          <Txt placeholder="e.g. 'very light', 'normal amount'…" value={weeNotes} onChange={(e) => setWeeNotes(e.target.value)} />
        </Field>
          ...
          <div style={{ marginTop: 16 }}>
            ...
            {wees.slice(0, 10).map((e) => (
              <EntryCard key={e.id} entry={e} color={{ base: "#6B9EC4", light: "#EEF6FF", dark: "#4A7FA8" }} onDelete={onDelete}
                summary={`Wee logged${e.notes ? ` · ${e.notes}` : ""}`} />
            ))}
          </div>
        )}
      </>}
...
// ─── MOOD ─────────────────────────────────────────────────────────────────────
const MOOD_OPTIONS = ["Very happy 😄", "Happy 😊", "Content 😌", "Unsettled 😕", "Fussy 😤", "Crying 😭"];
...
function MoodSection({ entries, onAdd, onDelete }) {
  ...
  return (
    <Section title="Daily Mood" emoji="🌤️" color={T.mood} count={entries.length}>
      <Field label="Date">
...
function ActivitySection({ entries, onAdd, onDelete }) {
  ...
  return (
    <Section title="Activities" emoji="🎯" color={T.activity} count={entries.length}>
        ...
        <div style={{ marginTop: 16 }}>
          {entries.slice(0, 10).map((e) => (
            <EntryCard key={e.id} entry={e} color={T.activity} onDelete={onDelete}
              summary={[e.activity, e.duration_mins && `${e.duration_mins} min`].filter(Boolean).join(" · ")} />
          ))}
        </div>
      )}
    </Section>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Real-time subscription — any change on any device updates both screens
  ...
  return (
    <div style={{ minHeight: "100vh", background: T.bg, fontFamily: "'Nunito', 'Trebuchet MS', sans-serif" }}>
      ...
      <div style={{
        ...
        <div style={{ color: "#C8B89A", fontSize: 13, marginTop: 4, marginBottom: 10 }}>Every feed · nap · smile · recorded</div>
      ...
      <div style={{ maxWidth: 520, margin: "0 auto", padding: "0 14px 52px" }}>
        ...
        {(syncStatus === "connecting" || (syncStatus === "syncing" && entries.length === 0)) && (
          <div style={{ textAlign: "center", padding: "32px 20px", color: T.muted }}>
            ...
            <div style={{ fontSize: 14 }}>Connecting to your shared database…</div>
          </div>
        )}
      </div>
    </div>
